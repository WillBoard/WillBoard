...
{
    public class BanAppealBoardCommandHandler : IRequestHandler<BanAppealBoardCommand, Status<InternalError>>
    {
        ...
        public BanAppealBoardCommandHandler(IpManager ipManager, IDateTimeProvider dateTimeProvider, IBanRepository banRepository, IBanAppealRepository banAppealRepository, IBoardCache boardCache, IBanCache banCache, IBanAppealCache banAppealCache, ILockManager lockManager)
        {
        ...
        public async ValueTask<Status<InternalError>> Handle(BanAppealBoardCommand request, CancellationToken cancellationToken)
        {
            ...
            var banCacheCollection = await _banCache.GetBoardUnexpiredCollectionAsync(board.BoardId, ipVersion, ipNumber);
            ...
            {
                var banRepositoryCollection = await _banRepository.ReadBoardUnexpiredCollectionAsync(board.BoardId, ipVersion, ipNumber);
                ...
                var banAppealRepositoryCollection = await _banAppealRepository.ReadBoardBanCollectionAsync(board.BoardId, ban.BanId);
                ...
                await _banAppealCache.RemoveBoardAsync(board.BoardId, newBanAppeal.BanAppealId);
                await _banAppealCache.RemoveBoardBanCollectionAsync(board.BoardId, newBanAppeal.BanId);
